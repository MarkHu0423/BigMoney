...
class SingleBackTest(object):
    ...
    def predict_action(self, kline):
        ...
        if self.side == 'long':
            ...
            if self.last_y < 30 and pred_y > 30 and self.pos_record['pos'] == 0:
                ...
                self.last_y = pred_y
                return 'close'
            elif self.last_y > 70 and pred_y < 70 and self.pos_record['pos'] == 1:
                self.last_y = pred_y
                return 'sell'
            elif self.pos_record['pos'] == -1 and pred_y < 30:
                self.last_y = pred_y
                return 'close'
            self.last_y = pred_y
        else:
    ...
    def analysis_drawpic(self, result):
        ...
        output_file(f"data/result_{tmp}_{ToolDateTime.get_date_string('s')}.html")
        show(column(fig))
...
if __name__ == '__main__':
    ...
    side = 'long'
    # side = 'short'
    bt = SingleBackTest({'data': list_data, 'open': 30, 'close': 70, 'side': side, 'factor': factor})
    result = bt.run()
